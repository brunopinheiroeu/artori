...
const AppHeader = ({ 
  ...
  const textGradient = isSchool ? 'from-green-600 to-blue-600' : 'from-indigo-600 to-purple-600';
    ...
    <header className="container mx-auto px-4 py-6">
      <nav className="flex items-center justify-between">
        <Link to={backLink} className="flex items-center space-x-2">
          ...
          <div>
            <span className={`text-2xl font-bold bg-gradient-to-r ${textGradient} bg-clip-text text-transparent`}>
              {brandName}
            </span>
            {isSchool && (
