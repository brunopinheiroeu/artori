...
async def get_questions(exam_id: str, subject_id: str):
    ...
    try:
        ...
        try:
            questions_cursor = db.questions.find({"subject_id": ObjectId(subject_id)})
            db_questions = list(questions_cursor)
            
            if not db_questions:
                logger.warning(f"No questions found for subject {subject_id}")
                return []
            
            # Convert database questions to API format
            api_questions = []
            for db_question in db_questions:
                # Convert options from database format to API format
                options = []
                for option in db_question.get("options", []):
                ...
                api_questions.append(Question(
                    id=str(db_question["_id"]),
                    subject_id=subject_id,
                    question=db_question.get("question", ""),
                    options=options
                ))
            
            logger.info(f"Retrieved {len(api_questions)} questions for subject {subject_id}")
            return api_questions
            
        except Exception as db_error:
            logger.error(f"Database error retrieving questions: {db_error}")
